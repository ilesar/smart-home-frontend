<template>
    <a-list itemLayout="horizontal" :dataSource="groceryList">
        <a-list-item slot="renderItem" slot-scope="item, index"
                     :style="{paddingRight: '24px', paddingLeft: $isMobile() ? '24px' : '0px'}">
            <a-list-item-meta
            ...
            <a-button type="default" shape="round" icon="edit"
                      @click="$isMobile() ? editMobileItem(item) : editItem(item)"
    ...
    export default class Groceries extends Vue {
        ...
        public editMobileItem(item: GroceryItem) {
            EventBus.$emit(EventBusEvents.OpenDrawer, {
                title: 'Uredi namirnicu',
                model: item,
                component: GroceryItemMobileFormEdit,
                submitText: 'Spremi',
